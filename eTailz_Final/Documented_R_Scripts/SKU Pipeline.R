...
for (i in 1:length(fileNames)) {
  ...
  TempFrame<-openxlsx::read.xlsx(fileNames[i], sheet = 1)
  if(nrow(TempFrame)>0){TempFrame$fileName<-SheetNames[i]}
...
#Deal with Rakutan
#CompiledListingStatus$Item.Status[
#  which(CompiledListingStatus$fileName=="RakutenAll")
#  ]<-mgsub(CompiledListingStatus$Status[
#    which(CompiledListingStatus$fileName=="RakutenAll")
#    ], c("Not Listed", "Listed"), c("Not Sent", "Sent"))
...
eBayClosed<-openxlsx::read.xlsx(eBayfileNames[grepl("closed", eBayfileNames, ignore.case = T)], sheet = 1)
eBayOpen<-openxlsx::read.xlsx(eBayfileNames[grepl("open", eBayfileNames, ignore.case = T)], sheet = 1)
eBayErrors<-openxlsx::read.xlsx(eBayfileNames[grepl("error", eBayfileNames, ignore.case = T)], sheet = 1)
...
UnityAmzProducts<-fetch(dbSendQuery(redshift, "Select p.ca_sku sku, p.ca_title title, pit.qty from etailz_unity.products p
                                    Join (Select pi.product_id, sum(pi.quantity) qty From etailz_unity.product_inventory pi
...
#Matching Vendors
Vendors<-fetch(dbSendQuery(redshift , "Select v.name, v.prefix, v.id unityid from etailz_unity.vendors v"),n=-1)
...
#Error Code Analysis
ErrorCodeClasses<-openxlsx::read.xlsx('P:/Biz Dev/Marketplace Expansion/ChannelAdvisor Exports/Error Classifications/Error Classifications.xlsx', sheet = 1)
